...
object MiscUtils : MinecraftInstance {
    ...
    fun generateCrashInfo(): String {
        var base = """
            --- Game crash info ---
              Client: ${AloxClient.CLIENT_NAME} ${AloxClient.clientVersionText} (${AloxClient.clientCommit})
    ...
    fun Throwable.showErrorPopup(
        ...
        val openIssueButton = JButton("Open GitHub Issue").apply {
            addActionListener {
                showURL("${AloxClient.CLIENT_GITHUB}/issues/new?template=bug_report.yml&title=%5BBUG%5D+Game+crashed+$exceptionType")
            }
        }
