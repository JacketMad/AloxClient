            ...
            if (swing && failSwing) {
                ...
                runWithModifiedRaycastResult(rotation, range.toDouble(), throughWallsRange.toDouble()) {
                    ...
                    val isAnyClientGuiActive = mc.currentScreen?.javaClass?.`package`?.name?.contains(
                        AloxClient.CLIENT_NAME, ignoreCase = true
                    ) == true
...
private fun updateHittable() {
    ...
    private fun shouldCancelDueToModuleState(): Boolean {
        return (blinkCheck && AloxClient.moduleManager[Blink::class.java.simpleName]?.state == true)
                || (noScaffold && AloxClient.moduleManager[Scaffold::class.java.simpleName]?.state == true)
                || (noFly && AloxClient.moduleManager[Flight::class.java.simpleName]?.state == true)
