...
import net.ccbluex.liquidbounce.AloxClient
...
object SettingsCommand : Command("autosettings", "autosetting", "settings", "setting", "config") {
    ...
    private suspend fun uploadSettings(args: Array<String>) {
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val name = "${AloxClient.clientCommit}-${serverData.serverIP.replace(".", "_")}"
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                loadSettings(false) { settingsList ->
                    ...
                    val configFiles = AloxClient.fileManager.settingsDir.listFiles { _, name ->
                        name.endsWith(".txt")
                    }
    ...
    private suspend fun deleteConfig(args: Array<String>) {
        withContext(Dispatchers.IO) {
            ...
            val file = File(AloxClient.fileManager.settingsDir, "$configName.txt")
    ...
    private suspend fun openFolder() {
        withContext(Dispatchers.IO) {
            Desktop.getDesktop().open(AloxClient.fileManager.settingsDir)
            chat("Opening folder...")
        }
    }
    ...
    private suspend fun renameConfig(args: Array<String>) {
        withContext(Dispatchers.IO) {
            ...
            val oldFile = File(AloxClient.fileManager.settingsDir, "$oldName.txt")
            val newFile = File(AloxClient.fileManager.settingsDir, "$newName.txt")
    ...
    private suspend fun currentConfig() {
        withContext(Dispatchers.IO) {
            alert("Current config is ${AloxClient.fileManager.nowConfig}")
